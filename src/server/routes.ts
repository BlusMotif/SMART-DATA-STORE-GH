...
export async function registerRoutes(
  ...
  // ============================================
  app.get("/api/dealer/stats", requireAuth, requireDealer, async (req, res) => {
    try {
      const dbUser = await storage.getUserByEmail(req.user!.email);
      if (!dbUser) {
        return res.status(404).json({ error: "User not found" });
      ...
      // Get all transactions for the dealer
      ...
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      const todayTransactions = allTransactions.filter(t => new Date(t.createdAt) >= today && t.status === 'completed');

      const todayProfit = todayTransactions.reduce((sum, tx) => sum + parseFloat(tx.profit || '0'), 0);
      const todayTransactionCount = todayTransactions.length;

      // Get total network size (dealer's agents)
      const networkAgents = await storage.getAgentsByDealer(dbUser.id);
      const networkSize = networkAgents.length;

      // Get total sales and profit from all completed transactions
      const completedTransactions = allTransactions.filter(t => t.status === 'completed');
      const totalSales = completedTransactions.reduce((sum, tx) => sum + parseFloat(tx.amount), 0);
      const totalProfit = parseFloat(agent.totalProfit || '0');
      ...
      const balance = Math.max(0, totalProfit - withdrawnTotal);
      ...
      res.status(500).json({ error: "Failed to load dealer stats" });
    }
  });

  app.get("/api/dealer/transactions/recent", requireAuth, requireDealer, async (req, res) => {
    try {
      const dbUser = await storage.getUserByEmail(req.user!.email);
      if (!dbUser) {
        return res.status(404).json({ error: "User not found" });
      ...
      // Get recent transactions from dealer's network
      const transactions = await storage.getTransactions({
        agentId: agent.id,
        limit: 10,
      });
      ...
      console.error("Error loading dealer transactions:", error);
      res.status(500).json({ error: "Failed to load dealer transactions" });
    }
  });

  // ============================================
  // SUPER-DEALER DASHBOARD ENDPOINTS
  // ============================================
  app.get("/api/super-dealer/stats", requireAuth, requireSuperDealer, async (req, res) => {
    try {
      const dbUser = await storage.getUserByEmail(req.user!.email);
      if (!dbUser) {
        return res.status(404).json({ error: "User not found" });
      ...
      const agent = await storage.getAgentByUserId(dbUser.id);
      if (!agent) {
        return res.status(404).json({ error: "Super dealer not found" });
      }

      // Get all transactions for the super dealer
      const allTransactions = await storage.getTransactions({
        agentId: agent.id
      });
      ...
      // Get network size (dealers and agents under super dealer)
      const dealers = await storage.getAgentsBySuperDealer(dbUser.id);
      const dealerCount = dealers.length;

      // Count all agents under these dealers
      let networkSize = dealerCount;
      for (const dealer of dealers) {
        const dealerAgents = await storage.getAgentsByDealer(dealer.userId);
        networkSize += dealerAgents.length;
      }
      ...
      console.error("Error loading super dealer stats:", error);
      res.status(500).json({ error: "Failed to load super dealer stats" });
    }
  });

  app.get("/api/super-dealer/transactions/recent", requireAuth, requireSuperDealer, async (req, res) => {
    try {
      const dbUser = await storage.getUserByEmail(req.user!.email);
      if (!dbUser) {
        return res.status(404).json({ error: "User not found" });
      ...
      // Get recent transactions from super dealer's network
      const transactions = await storage.getTransactions({
        agentId: agent.id,
        limit: 10,
      });
      ...
      console.error("Error loading super dealer transactions:", error);
      res.status(500).json({ error: "Failed to load super dealer transactions" });
    }
  });
  ...
  // ============================================
  app.get("/api/master/stats", requireAuth, requireMaster, async (req, res) => {
    try {
      const dbUser = await storage.getUserByEmail(req.user!.email);
      if (!dbUser) {
        return res.status(404).json({ error: "User not found" });
      ...
      // Get all transactions for the master
      const allTransactions = await storage.getTransactions({
        agentId: agent.id
      });
      ...
      // Get empire size (super dealers, dealers, and agents)
      const superDealers = await storage.getAgentsByMaster(dbUser.id);
      const superDealerCount = superDealers.length;

      let dealerCount = 0;
      let agentCount = 0;

      for (const superDealer of superDealers) {
        const dealers = await storage.getAgentsBySuperDealer(superDealer.userId);
        dealerCount += dealers.length;

        for (const dealer of dealers) {
          const agents = await storage.getAgentsByDealer(dealer.userId);
          agentCount += agents.length;
        }
      }

      const networkSize = superDealerCount + dealerCount + agentCount;
      ...
      res.status(500).json({ error: "Failed to load master stats" });
    }
  });

  app.get("/api/master/transactions/recent", requireAuth, requireMaster, async (req, res) => {
    try {
      const dbUser = await storage.getUserByEmail(req.user!.email);
      if (!dbUser) {
        return res.status(404).json({ error: "User not found" });
      ...
      // Get recent transactions from master's empire
      const transactions = await storage.getTransactions({
        agentId: agent.id,
        limit: 10,
      });
      ...
      console.error("Error loading master transactions:", error);
      res.status(500).json({ error: "Failed to load master transactions" });
    }
  });
