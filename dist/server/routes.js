                        ...
                        if (fulfillResult && fulfillResult.success && fulfillResult.results && fulfillResult.results.length > 0) {
                            ...
                            if (allSuccess) {
                                ...
                                await storage.updateTransaction(transaction.id, {
                                    ...
                                    completedAt: new Date(),
                                    paymentReference: data.reference,
                                    failureReason: `Provider rejected ${failedItems.length}/${fulfillResult.results.length} items: ${failedItems.map((r) => r.error || 'Unknown error').join(', ')}`,
                                    paymentStatus: "paid",
                                });
                            }
                        }
...
export async function registerRoutes(httpServer, app) {
            ...
            if (data.paymentMethod === 'wallet') {
                ...
                if (transaction.type === ProductType.RESULT_CHECKER && transaction.productId) {
                    ...
                    if (fulfillmentResult.success) {
                        // Order was placed successfully, set to processing until SkyTech delivers
                        await storage.updateTransaction(transaction.id, {
                            status: TransactionStatus.PENDING,
                            deliveryStatus: "processing",
                        });
    ...
    app.get("/api/transactions/verify/:reference", async (req, res) => {
        try {
            ...
            if (transaction.type === ProductType.RESULT_CHECKER && transaction.productId) {
                ...
                const requestedQuantity = transaction.quantity || 1;
                if (requestedQuantity > 1) {
                    // Handle multiple result checkers
                    console.log(`[Verify] Processing ${requestedQuantity} result checkers`);
                    const [type, year] = transaction.productId.split('-');
                    const checkers = await storage.getAvailableResultCheckersByQuantity(type, parseInt(year), requestedQuantity);
            ...
            // Get updated transaction to return actual status
        ...
        try {
            ...
            if (role === UserRole.AGENT ||
                ...
                if (agent) {
                    ...
                    }).from(transactions).where(and(eq(transactions.agentId, agent.id), or(eq(transactions.status, 'completed'), eq(transactions.paymentStatus, 'paid'))));
        ...
        try {
            ...
            if (role === UserRole.AGENT || role === UserRole.DEALER || role === UserRole.SUPER_DEALER || role === UserRole.MASTER) {
                ...
                if (agent) {
                    ...
                    const completedTransactions = await db.select().from(transactions).where(and(eq(transactions.agentId, agent.id), or(eq(transactions.status, 'completed'), eq(transactions.paymentStatus, 'paid'))));
    ...
    app.get("/api/agent/performance-history", requireAuth, requireAgent, async (req, res) => {
        try {
            ...
            for (let i = 6; i >= 0; i--) {
                ...
                const dayTransactions = await db.select().from(transactions).where(and(eq(transactions.agentId, agent.id), or(eq(transactions.status, 'completed'), eq(transactions.paymentStatus, 'paid')), gte(transactions.createdAt, startOfDay), lte(transactions.createdAt, endOfDay)));
    ...
    app.get("/api/agent/wallet", requireAuth, requireAgent, async (req, res) => {
        try {
            ...
            // Get last topup
            const lastTopup = walletTopups.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())[0];
        ...
        try {
            ...
            const csvData = transactions.map(tx => ({
                ...
                phoneNumbers: (tx.phoneNumbers && Array.isArray(tx.phoneNumbers)) ?
                    tx.phoneNumbers.map((p) => p.phone).join("; ") :
                    "",
    ...
    app.get("/api/admin/users", requireAuth, requireAdmin, async (req, res) => {
        try {
            ...
            const usersWithPurchaseData = await Promise.all(allUsers.map(async (user) => {
                ...
                const totalSpent = sortedTransactions.reduce((sum, t) => sum + parseFloat(t.amount), 0);
                return {
                    id: user.id,
                    name: user.name,
                    email: user.email,
    ...
    app.post("/api/webhooks/paystack", async (req, res) => {
        try {
            ...
            const isValid = validateWebhookSignature(JSON.stringify(req.body), req.headers["x-paystack-signature"]);
            if (!isValid) {
    ...
    app.post("/api/wallet/pay", requireAuth, async (req, res) => {
        try {
            ...
            const phoneNumbersData = orderItems
                ...
                : (phoneNumbers ? phoneNumbers.map((phone) => ({ phone })) : undefined);
    ...
    app.post("/api/support/chat/create", requireAuth, async (req, res) => {
        try {
            const user = req.user;
    ...
    app.get("/api/support/chat/:chatId", requireAuth, async (req, res) => {
        try {
            const user = req.user;
    ...
    app.post("/api/support/chat/:chatId/message", requireAuth, async (req, res) => {
        try {
            const user = req.user;
            const { chatId } = req.params;
    ...
    app.put("/api/support/message/:messageId/read", requireAuth, async (req, res) => {
        try {
    ...
    // Close chat
    app.put("/api/support/chat/:chatId/close", requireAuth, async (req, res) => {
        try {
            const user = req.user;
            const { chatId } = req.params;
            const chat = await storage.getSupportChatById(chatId);
    ...
    app.get("/api/support/admin/unread-count", requireAuth, requireSupport, async (req, res) => {
        try {
    ...
    app.get("/api/admin/support/chats", requireAuth, requireAdmin, async (req, res) => {
        try {
            const { status } = req.query;
    ...
    app.put("/api/admin/support/chat/:chatId/assign", requireAuth, requireAdmin, async (req, res) => {
        try {
            const { chatId } = req.params;
            const adminId = req.user.id;
    ...
    app.put("/api/admin/users/:userId/credentials", requireAuth, requireAdmin, async (req, res) => {
        try {
    ...
    app.get("/api/admin/external-prices", requireAuth, requireAdmin, async (req, res) => {
        try {
            ...
            const result = await getExternalPrices(network, min_capacity ? parseInt(min_capacity) : undefined, max_capacity ? parseInt(max_capacity) : undefined, effective ? effective === '1' || effective === 'true' : undefined, providerId);
            if (result.success) {
                res.json(result);
            }
            else {
    ...
    app.post('/api/cron/cleanup-failed-orders', async (req, res) => {
        try {
