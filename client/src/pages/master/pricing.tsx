...
import { AgentSidebarV2 as MasterSidebar } from "@/components/layout/agent-sidebar-v2";
...
export default function MasterPricingPage() {
  ...
  const [priceChanges, setPriceChanges] = useState<Record<string, { masterPrice: string; masterProfit: string }>>({});
  ...
  const updateMutation = useMutation({
    mutationFn: (prices: Record<string, { masterPrice: string; masterProfit: string }>) =>
      apiRequest("POST", "/api/master/pricing", { prices }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/master/pricing"] });
      setPriceChanges({});
  ...
  const getCurrentPrice = (bundleId: string) => {
    ...
    return {
      masterPrice: changes?.masterPrice || existing?.masterPrice || "0.00",
      masterProfit: changes?.masterProfit || existing?.masterProfit || "0.00",
      adminBasePrice: existing?.adminBasePrice || "0.00",
    };
  };

  const handlePriceChange = (bundleId: string, field: 'masterPrice' | 'masterProfit', value: string) => {
    const current = getCurrentPrice(bundleId);
    const numValue = parseFloat(value) || 0;

    if (field === 'masterPrice') {
      // When master price changes, calculate profit
      ...
      setPriceChanges(prev => ({
        ...prev,
        [bundleId]: {
          masterPrice: sellingPrice.toFixed(2),
          masterProfit: value,
        }
      }));
    }
                          ...
                          <TableRow key={price.bundleId} className={hasChanges ? "bg-blue-50" : ""}>
                            ...
                            <TableCell>
                              <Input
                                type="number"
                                step="0.01"
                                value={current.masterPrice}
                                onChange={(e) => handlePriceChange(price.bundleId, 'masterPrice', e.target.value)}
                            ...
                            <TableCell>
                              <Input
                                type="number"
                                step="0.01"
                                value={current.masterProfit}
                                onChange={(e) => handlePriceChange(price.bundleId, 'masterProfit', e.target.value)}
                                className="w-24 tabular-nums"
                                min="0"
                              />
                            </TableCell>
